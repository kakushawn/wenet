...
# dev_set=train_dev
train_dir=data/matbn/train
test_dir=data/matbn/test
dev_dir=data/matbn/dev
bpe_model=local/resource/m_vocab1000.model
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  tools/compute_cmvn_stats.py --num_workers 16 --train_config $train_config \
    --in_scp $train_dir/wav.scp \
    --out_cmvn $train_dir/global_cmvn
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  if [ -d $dict ]; then rm -r $dict; fi
  mkdir -p $(dirname $dict)
  echo "<blank> 0" > ${dict} # 0 will be used for "blank" in CTC
  # echo "<unk> 1" >> ${dict} # <unk> must be 1

  cp $train_dir/text $train_dir/text4dict
  sed -i 's/\xEF\xBB\xBF//' $train_dir/text4dict

  tools/text2token.py -s 1 -n 1 -m ${bpecode} \
    $train_dir/text4dict ${trans_type_ops} \
    | cut -f 2- -d " " | tr " " "\n" \
  ...
  for x in ${dev_set} ${train_set} test; do
    if [ $data_type == "shard" ]; then
      tools/make_shard_list.py --num_utts_per_shard $num_utts_per_shard \
        --num_threads 16 $train_dir/wav.scp \
        $train_dir/text \
        $train_dir/shards \
        $train_dir/data.list
    else
      tools/make_raw_list.py $train_dir/wav.scp \
      $train_dir/text \
      $train_dir/data.list
    fi
  done
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  cmvn_opts=
  $cmvn && cp $train_dir/global_cmvn $dir
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python wenet/bin/train.py --gpu $gpu_id \
      ...
      --data_type $data_type \
      --symbol_table $dict \
      --train_data $train_dir/data.list \
      --cv_data $dev_dir/data.list \
